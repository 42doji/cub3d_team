...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd >= MAX_FD)
		return (NULL);
	if (!g_left_str[fd])
		g_left_str[fd] = ft_strdup("");
	if (!g_left_str[fd])
		return (NULL);
	g_left_str[fd] = read_to_left_str(fd, g_left_str[fd]);
	if (!g_left_str[fd])
	{
		g_left_str[fd] = NULL;
		return (NULL);
	}
	line = ft_get_line(g_left_str[fd]);
	if (!line)
	{
		free(g_left_str[fd]);
		g_left_str[fd] = NULL;
	...
	g_left_str[fd] = ft_new_left_str(g_left_str[fd]);
	return (line);
}
...
void	get_next_line_cleanup(int fd)
{
	...
	else if (fd >= 0 && fd < MAX_FD && g_left_str[fd])
	{
		free(g_left_str[fd]);
		g_left_str[fd] = NULL;
	}
